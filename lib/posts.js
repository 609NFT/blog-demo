...
// Seed content: written to the JSON store on first run, then fully editable
// through /admin. Editing here only affects brand-new deploys.
const SEED = [
  {
...
// ---- File-backed store -----------------------------------------------------
// Posts live in a JSON file on the app's persistent workspace (gitignored), so
// edits made in /admin survive restarts and a deploy never clobbers real
// content. Seeded from SEED on first run.
...
function readAll() {
  ...
  try {
    ...
    // Preserve the unreadable file for inspection, then reseed rather than crash.
    const backup = FILE + '.corrupt.' + Date.now();
    try { fs.renameSync(FILE, backup); } catch (_) {}
    console.error(`posts.json unreadable (${err.message}); moved to ${backup}`);
    ensure();
...
function writeAll(rows) {
  ...
  // Atomic write: full temp file then rename, so a crash mid-write can't
  // truncate posts.json.
  const tmp = FILE + '.tmp';
...
function slugify(s) {
  return String(s).toLowerCase().replace(/[^\w\s-]/g, '').trim().replace(/\s+/g, '-').replace(/-+/g, '-').slice(0, 80) || 'post';
}

function uniqueSlug(base, exceptSlug) {
  const rows = readAll();
  let slug = base, n = 1;
  while (rows.some((p) => p.slug === slug && p.slug !== exceptSlug)) { n += 1; slug = `${base}-${n}`; }
  return slug;
}

// Coerce a form/API payload into a clean, bounded post record.
function normalize(input) {
  const s = (v) => String(v == null ? '' : v);
  const rawTags = Array.isArray(input.tags) ? input.tags : s(input.tags).split(',');
  return {
    title: s(input.title).trim(),
    category: (s(input.category).trim().toLowerCase().replace(/\s+/g, '-') || 'general'),
    author: (s(input.author).trim() || 'Anonymous'),
    date: /^\d{4}-\d{2}-\d{2}$/.test(input.date) ? input.date : new Date().toISOString().slice(0, 10),
    tags: rawTags.map((t) => s(t).trim().toLowerCase().replace(/\s+/g, '-')).filter(Boolean).slice(0, 8),
    excerpt: s(input.excerpt).trim().slice(0, 400),
    body: s(input.body).slice(0, 40000),
  };
}
...
function create(input) {
  ...
  if (!data.body.trim()) throw new Error('body_required');
  const rows = readAll();
  const slug = uniqueSlug(slugify(input.slug || data.title));
...
function update(slug, input) {
  ...
  if (i === -1) return null;
  const data = normalize({ ...rows[i], ...input });
  if (!data.title) throw new Error('title_required');
  let newSlug = slug;
  if (input.slug && slugify(input.slug) !== slug) newSlug = uniqueSlug(slugify(input.slug), slug);
  rows[i] = { slug: newSlug, ...data };
  writeAll(rows);
  return rows[i];
}
...
function categories() {
  const counts = {};
  readAll().forEach((p) => { counts[p.category] = (counts[p.category] || 0) + 1; });
...
function tags() {
  const counts = {};
  readAll().forEach((p) => (p.tags || []).forEach((t) => { counts[t] = (counts[t] || 0) + 1; }));
  return Object.entries(counts)
    .map(([slug, count]) => ({ slug, count }))
    .sort((a, b) => b.count - a.count || a.slug.localeCompare(b.slug));
}
...
function related(slug, limit = 3) {
  ...
  const ranked = readAll().filter((p) => p.slug !== slug)
    .map((p) => {
...
function adjacent(slug) {
  const ordered = readAll().slice().sort((a, b) => (a.date < b.date ? 1 : -1));
  const i = ordered.findIndex((p) => p.slug === slug);
  return { newer: ordered[i - 1] ? toCard(ordered[i - 1]) : null, older: ordered[i + 1] ? toCard(ordered[i + 1]) : null };
}

function all() { return readAll().slice().sort((a, b) => (a.date < b.date ? 1 : -1)); }

module.exports = { list, getBySlug, categories, tags, search, related, adjacent, all, create, update, remove };
